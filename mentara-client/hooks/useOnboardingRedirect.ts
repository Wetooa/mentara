  ...
  useEffect(() => {
    ...
    function checkOnboardingStatus() {
      ...
      const shouldRedirectToOnboarding =
        isFirstTimeUser || (!onboardingCompleted && !hasBackendProfile);
      ...
      if (shouldRedirectToOnboarding) {
        console.log(
          "Redirecting to onboarding - First time user or incomplete onboarding"
        );
        router.push("/user/onboarding/profile");
      } else if (shouldRedirectToWelcome) {
        console.log(
          "Redirecting to welcome page - Onboarding complete but welcome not seen"
        );
