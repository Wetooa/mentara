...
export default function ClientWelcomePage() {
  ...
  const [currentStep, setCurrentStep] = useState<'loading' | 'recommendations' | 'selection' | 'sending'>('loading');
  ...
  const sendRequestsMutation = useMutation({
    mutationFn: (therapistIds: string[]) => 
      api.therapists.sendTherapistRequests({ therapistIds }),
    onSuccess: () => {
  ...
  const handleSendRequests = async () => {
    ...
    await sendRequestsMutation.mutateAsync(selectedTherapists);
  };
