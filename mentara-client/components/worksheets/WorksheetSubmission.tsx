...
interface WorksheetSubmissionProps {
  ...
  onSubmit: (responses: WorksheetResponse[], attachments: AttachedFile[]) => Promise<void>;
  onSaveDraft: (responses: WorksheetResponse[], attachments: AttachedFile[]) => Promise<void>;
  disabled?: boolean;
  className?: string;
}
...
export function WorksheetSubmission({
  ...
  const [responses, setResponses] = useState<Record<string, WorksheetResponse>>(
    existingResponses.reduce((acc, response) => {
  ...
  const updateResponse = (questionId: string, update: Partial<WorksheetResponse>) => {
    setResponses(prev => ({
      ...prev,
      [questionId]: {
        ...prev[questionId],
        questionId,
        type: worksheet.questions.find(q => q.id === questionId)?.type || "text",
        ...update,
      }
    }));
  };
  ...
  const updateAttachments = (questionId: string, files: AttachedFile[]) => {
    ...
    const uploadedIds = files.filter(f => f.isUploaded && f.id).map(f => f.id!);
    ...
    for (const question of worksheet.questions) {
      ...
      switch (question.type) {
        case "text":
          ...
          if (question.minLength && response.textResponse.length < question.minLength) {
  ...
  const isOverdue = worksheet.dueDate && new Date() > new Date(worksheet.dueDate);
  const canEdit = worksheet.status !== "submitted" && !disabled;
    ...
    >
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            ...
            <Badge className={getStatusColor(worksheet.status)}>
              ...
              <span className="ml-1 capitalize">{worksheet.status.replace("_", " ")}</span>
            </Badge>
          </div>
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            {worksheet.dueDate && (
              <div className="flex items-center gap-1">
                ...
                <span>Due {format(new Date(worksheet.dueDate), "MMM dd, yyyy")}</span>
            ...
            {worksheet.submittedAt && (
              <div className="flex items-center gap-1">
                ...
                <span>Submitted {format(new Date(worksheet.submittedAt), "MMM dd, yyyy")}</span>
              </div>
            )}
          </div>
        </CardHeader>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  {question.maxLength && (
                    <div className="text-xs text-muted-foreground text-right">
                      {responses[question.id]?.textResponse?.length || 0}/{question.maxLength}
                    </div>
                  )}
                  {question.minLength && (
                  ...
                  {question.options.map((option, optionIndex) => (
                    ...
                    >
                      <input
                        ...
                        value={option}
                        checked={responses[question.id]?.selectedOptions?.includes(option)}
                        onChange={(e) => {
                          if (e.target.checked) {
                  ...
                  <div className="flex items-center gap-2">
                    {Array.from(
                      ...
                      <button
                        key={value}
                        type="button"
                        onClick={() => updateResponse(question.id, { numberResponse: value })}
                        disabled={!canEdit}
                        className={cn(
              ...
              {question.type === "file_upload" && (
                <FileAttachment
                  ...
                  onFilesChange={(files) => updateAttachments(question.id, files)}
                  maxFiles={3}
                  maxFileSize={10}
                  allowedTypes={question.allowedFileTypes || ["image/*", "application/pdf", "text/*"]}
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div className="text-sm text-muted-foreground">
                {Object.keys(responses).length} of {worksheet.questions.filter(q => q.required).length} required questions answered
              </div>
      ...
      {worksheet.status === "submitted" && (
        <Alert>
          ...
          <AlertDescription>
            This worksheet has been submitted successfully. Your therapist will review your responses.
          </AlertDescription>
        </Alert>
      )}
    </motion.div>
