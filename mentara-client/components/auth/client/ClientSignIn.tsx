...
import { useClientAuth } from "@/hooks/auth/client";
...
type ClientSignInForm = z.infer<typeof clientSignInSchema>;
...
export function ClientSignIn() {
  ...
  const { setLoadingWithDefaults, isLoading: isAuthLoading, getLoadingMessage } = useAuthLoadingStates();

  const form = useForm<ClientSignInForm>({
  ...
  const onSubmit = useCallback(async (data: ClientSignInForm) => {
    try {
      setFieldErrors({});
      ...
      // Navigation handled by useClientAuth hook
    } catch (err) {
      setLoadingWithDefaults("login", false);
  ...
  }, [login, handleAuthError, handleRetry, setLoadingWithDefaults, router, retryCount]);
      ...
      <CardContent className="space-y-4">
        ...
        {retryCount >= 3 && !isRetrying && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Multiple connection attempts failed. Please check your internet connection and try again.
            </AlertDescription>
          </Alert>
        )}
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      disabled={isLoading || isAuthLoading("login") || isRetrying}
            ...
            <Button
              ...
              disabled={isLoading || isAuthLoading("login") || isRetrying}
            >
              {(isLoading || isAuthLoading("login") || isRetrying) ? (
                <>
