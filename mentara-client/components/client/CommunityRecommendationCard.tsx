...
export function CommunityRecommendationCard({
  ...
  const getScoreColor = (score: number) => {
    if (score >= 0.8) return 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg';
    if (score >= 0.6) return 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white shadow-lg';
    if (score >= 0.4) return 'bg-gradient-to-r from-yellow-500 to-amber-500 text-white shadow-lg';
  ...
  return (
    <Card className={`transition-all duration-300 cursor-pointer group relative overflow-hidden ${
      isSelected 
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge 
                  className={`${getScoreColor(community.compatibilityScore)} px-3 py-1 font-bold text-sm transform group-hover:scale-105 transition-transform duration-300`}
                >
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 px-3 py-1 bg-white/70 rounded-lg border border-emerald-100 shadow-sm">
                  ...
                  <span className="text-sm font-medium text-gray-700">{community.memberCount} members</span>
                </div>
                <div className="flex items-center gap-2 px-3 py-1 bg-white/70 rounded-lg border border-blue-100 shadow-sm">
                  <TrendingUp className="h-4 w-4 text-blue-600" />
                  <span className="text-sm font-medium text-gray-700">{getScoreLabel(community.compatibilityScore)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 pt-4 mt-4 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-white/70 rounded-lg border border-emerald-100">
                <span className="font-bold text-gray-900 text-sm">Community Type:</span>
                <p className="text-emerald-600 font-medium text-sm">Support Group</p>
              </div>
              <div className="p-3 bg-white/70 rounded-lg border border-blue-100">
                <span className="font-bold text-gray-900 text-sm">Activity Level:</span>
