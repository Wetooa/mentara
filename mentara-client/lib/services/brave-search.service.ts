...
export class BraveSearchService {
  ...
  private readonly cache = new Map<
    string,
    { data: ExternalSearchResult[]; timestamp: number }
  >();
  ...
  async searchPeople(
    ...
    try {
      ...
      const braveResults = await this.callBraveSearchMCP(
        enhancedQuery,
        options
      );
  ...
  private enhanceQueryForPeople(
    ...
    if (options.searchType === "therapists") {
      enhancedQuery +=
        " therapist psychologist counselor mental health professional";
    } else {
  ...
  private generateMockResults(
    ...
    const baseQuery = query
      .replace(
        /\s+(therapist|psychologist|counselor|person|profile|contact|linkedin|professional|location|address)/gi,
        ""
      )
      .trim();
  ...
  ): "person" | "professional" | "organization" {
    const content =
      `${result.title} ${result.description || ""} ${result.snippet || ""}`.toLowerCase();

    if (
      content.includes("therapist") ||
      content.includes("psychologist") ||
  ...
  private calculateRelevanceScore(result: BraveSearchResult): number {
    ...
    if (
      result.url.includes("psychologytoday.com") ||
      result.url.includes("therapistfinder.com")
    ) {
      score += 0.25;
    }
