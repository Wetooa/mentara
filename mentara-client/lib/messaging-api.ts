...
export const createMessagingApiService = (getToken: () => Promise<string | null>) => {
  ...
  return {
    ...
    async fetchConversation(conversationId: string): Promise<Conversation | undefined> {
      try {
        const currentUserId = await getCurrentUserId();
        const messages = await makeRequest(`/messaging/conversations/${conversationId}/messages`);
    ...
    async removeMessageReaction(messageId: string, emoji: string): Promise<void> {
      try {
        await makeRequest(`/messaging/messages/${messageId}/reactions/${emoji}`, {
          method: 'DELETE',
        });
      } catch (error) {
    ...
    async searchMessages(query: string, conversationId?: string): Promise<Message[]> {
      try {
        ...
        const messages = await makeRequest(`/messaging/search?${params.toString()}`);
...
class MessagingApiService {
  constructor() {
    console.warn('MessagingApiService is deprecated. Use createMessagingApiService with authentication.');
  }
  
  async fetchContacts(): Promise<Contact[]> {
    console.error('MessagingApiService requires authentication. Use createMessagingApiService.');
    return [];
  }
  ...
  async sendMessage(): Promise<Message> {
    throw new Error('MessagingApiService requires authentication. Use createMessagingApiService.');
  }
