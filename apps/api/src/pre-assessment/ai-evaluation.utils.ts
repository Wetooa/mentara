...
function calculateOverallConfidence(
  ...
  for (const severity of severities) {
    ...
    const confidenceRange = SEVERITY_CONFIDENCE_MAP[severityLower] || [
      0.7, 0.9,
    ];
...
function generateRiskFactors(
  ...
  for (const [questionnaire, severity] of Object.entries(severityLevels)) {
    ...
    if (
      ['minimal', 'none', 'no', 'negative'].some((skip) =>
    ...
    for (const [riskFactor, relatedQuestionnaires] of Object.entries(
      RISK_FACTORS_POOL,
    )) {
  ...
  const maxRiskFactors = Math.min(
    Math.max(2, Math.ceil(riskFactorsArray.length * 0.6)),
    5,
  );
...
function generateRecommendations(
  ...
  for (const [questionnaire, severity] of Object.entries(severityLevels)) {
    ...
    for (const [recommendation, relatedConditions] of Object.entries(
      RECOMMENDATIONS_POOL,
    )) {
      if (
        relatedConditions.includes(questionnaire) ||
        relatedConditions.some((condition) =>
          severity.toLowerCase().includes(condition),
        )
      ) {
  ...
  const maxRecommendations = Math.min(
    Math.max(2, Math.ceil(recommendationsArray.length * 0.7)),
    4,
  );
 ...
 */
function convertSeverityToNormalizedScore(
  severity: string,
  ...
  if (severityLower.includes('none') || severityLower.includes('minimal'))
    return Math.random() * 0.2 + 0.0;
 ...
 */
function calculateOverallSeverityLevel(
  severityLevels: Record<string, string>,
): string {
  const severities = Object.values(severityLevels);
  ...
  for (const severity of severities) {
    ...
    if (
      ...
      severityLower.includes('very severe')
    )
      severityPoints += 5;
      ...
      severityLower.includes('positive')
    )
      severityPoints += 3;
