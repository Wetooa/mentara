...
export class AdminTherapistService {
  ...
  async getApplications(filters: PendingTherapistFiltersDto) {
    try {
      ...
      // Calculate summary statistics (optimized - single query instead of 3)
  ...
  // Alias for backward compatibility - both routes use the same method
  ...
  async updateTherapistStatus(
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        this.logger.debug(`Admin ID: ${adminId}`);
