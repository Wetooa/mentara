...
interface TherapistRecentCommunicationsProps {
  recentContacts: Contact[];
  onViewAllMessages?: () => void;
  onContactSelect?: (contactId: string) => void;
}

export default function TherapistRecentCommunications({
  recentContacts,
  onViewAllMessages,
  onContactSelect,
}: TherapistRecentCommunicationsProps) {
  const getDisplayTime = (time: string | undefined): string => {
    if (!time) {
      return "Unknown";
    }
  ...
  return (
    ...
    >
      <Card className="shadow-sm hover:shadow-md transition-shadow overflow-hidden p-0 h-full flex flex-col">
        <div className="bg-gradient-to-br from-blue-500/5 to-cyan-500/5 px-5 py-4 border-b border-border/50">
