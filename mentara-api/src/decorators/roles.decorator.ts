...
export const TherapistOrAdmin = () => SetMetadata(ROLES_KEY, ['therapist', 'admin']);
...
export const AdminOrResourceOwner = (paramName: string = 'id') => (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
...
export const TherapistOrResourceOwner = (paramName: string = 'id') => (target: any, propertyKey: string, descriptor: PropertyDescriptor) => {
  SetMetadata(ROLES_KEY, ['therapist', 'admin'])(target, propertyKey, descriptor);
  SetMetadata(RESOURCE_OWNER_KEY, paramName)(target, propertyKey, descriptor);
};
...
export const AssessmentAccess = () => 
  RequirePermissions('assessments:read:own', 'assessments:read:assigned', 'assessments:read:all');

// Moderation access (moderators and admins)
export const ModerationAccess = () => 
  RequirePermissions('posts:moderate', 'communities:moderate', 'messages:moderate');