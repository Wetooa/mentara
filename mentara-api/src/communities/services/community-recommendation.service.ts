...
export class CommunityRecommendationService {
   ...
   */
  async getUserRecommendations(userId: string): Promise<CommunityRecommendation[]> {
    try {
      ...
      for (const community of communities) {
        ...
        if (compatibilityScore > 0.3) { // Only recommend if score is decent
          const now = new Date();
  ...
  private async calculateCompatibilityScore(assessmentResponses: any[], community: any): Promise<number> {
  ...
  private generateRecommendationReason(score: number): string {
    if (score > 0.8) return 'Excellent match based on your assessment responses';
   ...
   */
  async generateRecommendationsForUser(userId: string, force = false): Promise<void> {
   ...
   */
  async getRecommendationById(recommendationId: string): Promise<CommunityRecommendation | null> {
   ...
   */
  async handleRecommendationInteraction(data: { recommendationId: string; action: string; userId: string }): Promise<void> {
