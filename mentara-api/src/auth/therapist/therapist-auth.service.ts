...
import { hashPassword, checkEmailAvailable } from '../shared/auth.helpers';
...
export class TherapistAuthService {
  ...
  async registerTherapistWithDocuments(
    ...
    await checkEmailAvailable(this.prisma, registerData.email);
    ...
    try {
      ...
      this.logger.log('Therapist registration success email sent successfully');
    } catch (error) {
      this.logger.error(
    ...
    try {
      ...
      if (updateData.status === 'APPROVED') {
        ...
        try {
          ...
          this.logger.log('Clerk account created successfully:', clerkUserId);
          ...
          this.logger.log('User role updated to therapist');
        } catch (error) {
          this.logger.error('Error creating Clerk account:', error);
        ...
        try {
          ...
          this.logger.log('Approval email notification sent successfully');
        } catch (error) {
          this.logger.error(
        ...
        try {
          ...
          this.logger.log('Rejection email notification sent successfully');
        } catch (error) {
          this.logger.error(
  ...
  private async createClerkTherapistAccount(
    ...
    try {
      this.logger.log('Creating Clerk account for therapist:', {
        email,
        fullName,
      });
      this.logger.log(
      ...
      this.logger.log('Clerk account created successfully:', {
        id: clerkUser.id,
