...
export class RoleBasedAccessGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const minimumRole = this.reflector.getAllAndOverride<keyof typeof ROLE_HIERARCHY>('min_role', [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    const resourceOwnerParam = this.reflector.getAllAndOverride<string>('resource_owner_param', [
      context.getHandler(),
      context.getClass(),
    ]);
  ...
  private checkRole(userRole: string, minimumRole: keyof typeof ROLE_HIERARCHY): boolean {
  ...
  private checkResourceOwnership(
    ...
    if (userRole === 'moderator') {
      ...
      if (path.includes('/communities') || path.includes('/posts') || path.includes('/comments')) {
        return true;
      }
    }
